    ...
    <div class="col-md-6">
        ...
        <select name="id_tipo_reserva" class="form-select" required>
            ...
            @foreach ($tipos as $tipo)
                <option value="{{ $tipo['id_tipo_reserva'] }}" {{ (isset($reserva['id_tipo_reserva']) && $reserva['id_tipo_reserva'] == $tipo['id_tipo_reserva']) ? 'selected' : '' }}>
                    {{ $tipo['descripcion'] }}
                </option>
            @endforeach
        </select>
    ...
    <div class="col-md-6">
        ...
        <select name="id_destino" class="form-select" required>
            ...
            @foreach ($destinos as $dest)
                <option value="{{ $dest['id_zona'] }}" {{ (isset($reserva['id_destino']) && $reserva['id_destino'] == $dest['id_zona']) ? 'selected' : '' }}>
                    {{ $dest['descripcion'] }}
                </option>
            @endforeach
        </select>
    ...
    <div class="col-md-6">
        ...
        <select name="id_vehiculo" class="form-select" required>
            ...
            @foreach ($vehiculos as $veh)
                <option value="{{ $veh['id_vehiculo'] }}" {{ (isset($reserva['id_vehiculo']) && $reserva['id_vehiculo'] == $veh['id_vehiculo']) ? 'selected' : '' }}>
                    {{ $veh['descripcion'] }}
                </option>
            @endforeach
        </select>
